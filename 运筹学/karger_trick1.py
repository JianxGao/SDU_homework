...
def trick1(G,alpha):
    ...
    if V <= 6:
        ...
        t = int(1 + V / np.sqrt(2))
        G1 = Karger_algorithm(G, t)
        G2 = Karger_algorithm(G, t/alpha)
        return min(trick1(G1,alpha),trick1(G2,alpha))


@calcute_total_time
def karger_stein_algorithm(G, alpha,iteration):
    reuslt_list = []
    for i in range(iteration):
        min_cut = trick1(G,alpha)
...
if __name__ == '__main__':
    ...
    alpha=2
    iteration = 10
    improved_result = karger_stein_algorithm(G,alpha,iteration)
