...
def get_feature(data, total_data, fft_b_g, total_fft, label):
    ...
    for i in range(length):
        ...
        for j in range(9):
            ...
            dataset[i].append(np.sum(np.square(data[i][j])) /
                              len(data[i][j]))  # acc_body_energy
        ...
        for j in range(3):
            ...
            # acc_total_data_energy
            dataset[i].append(
                np.sum(np.square(total_data[i][j])) / len(total_data[i][j]))
            # dataset[i].append(
        ...
        dataset[i].append(np.corrcoef(
            total_data[i][0], total_data[i][1])[0][1])
        dataset[i].append(np.corrcoef(
            total_data[i][0], total_data[i][2])[0][1])
        dataset[i].append(np.corrcoef(
            total_data[i][1], total_data[i][2])[0][1])
    ...
    # FFT
        ...
        for j in range(9):
            ...
            dataset[i].append(np.sum(np.square(fft_b_g[i][j])) /
                              len(fft_b_g[i][j]))  # fft_body_energy
