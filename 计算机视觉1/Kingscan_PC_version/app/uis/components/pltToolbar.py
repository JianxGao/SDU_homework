...
from uis.components import backend_tools, cbook
from uis.components._pylab_helpers import Gcf
from uis.components.backend_bases import (
...
from uis.components.qt_compat import (
...
class FigureCanvasQT(QtWidgets.QWidget, FigureCanvasBase):
    ...
    def _get_key(self, event):
        ...
        try:
            ...
            # unicode defines data points up to 0x10ffff (sys.maxunicode)
    ...
    def draw(self):
        ...
        # The renderer draw is done here; delaying causes problems with data
        # that uses the results of the draw() to update plot elements.
        if self._is_drawing:
            return
...
class FigureManagerQT(FigureManagerBase):
    ...
    def _get_toolbar(self, canvas, parent):
        ...
        if matplotlib.rcParams['toolbar'] == 'toolbar2':
            toolbar = NavigationToolbar2QTself(canvas, parent, True)
...
class NavigationToolbar2QTself(NavigationToolbar2, QtWidgets.QToolBar):
    ...
    def __init__(self, canvas, parent, coordinates=True):
        ...
        self.setIconSize(QtCore.QSize(30, 30)) # 树莓派加上
        for text, tooltip_text, image_file, callback in self.toolitems[:-2]:
            if text is None:
                self.addSeparator()
            else:
        ...
        # icon = QtGui.QIcon()
        # icon.addPixmap(QtGui.QPixmap("material/edit.png"), QtGui.QIcon.Selected, QtGui.QIcon.On)
        # self.edit_action = QtWidgets.QAction(self)
        # self.edit_action.setIcon(icon)
        # self.edit_action.setCheckable(True)
        # self.edit_action.setObjectName("edit_action")
        # self.edit_action.setToolTip("Add notch filter points")
        # self.addAction(self.edit_action)
        # self._actions['edit'] = self.edit_action
...
class ToolbarQt(ToolContainerBase, QtWidgets.QToolBar):
    ...
    def add_toolitem(
        ...
        if image_file:
            button.setIcon(NavigationToolbar2QTself._icon(self, image_file))
        button.setText(name)
        if description:
...
class ConfigureSubplotsQt(backend_tools.ConfigureSubplotsBase):
    def trigger(self, *args):
        NavigationToolbar2QTself.configure_subplots(
            self._make_classic_style_pseudo_toolbar())


class SaveFigureQt(backend_tools.SaveFigureBase):
    def trigger(self, *args):
        NavigationToolbar2QTself.save_figure(
            self._make_classic_style_pseudo_toolbar())


class SetCursorQt(backend_tools.SetCursorBase):
    def set_cursor(self, cursor):
        NavigationToolbar2QTself.set_cursor(
            self._make_classic_style_pseudo_toolbar(), cursor)


class RubberbandQt(backend_tools.RubberbandBase):
    def draw_rubberband(self, x0, y0, x1, y1):
        NavigationToolbar2QTself.draw_rubberband(
            self._make_classic_style_pseudo_toolbar(), None, x0, y0, x1, y1)

    def remove_rubberband(self):
        NavigationToolbar2QTself.remove_rubberband(
            self._make_classic_style_pseudo_toolbar())
