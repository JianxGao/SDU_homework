...
class CallbackRegistry:
    ...
    However, real data in Matplotlib rarely does so, and due to its design,
    it is rather difficult to place this kind of data.  To get around this,
...
def _check_1d(x):
    ...
    if not hasattr(x, 'shape') or len(x.shape) < 1:
        ...
        try:
            ...
            #
            # This data should correctly identify and coerce to a
            ...
            # something where 2D slicing does not results in a 2D
            # object.
            if ndim < 2:
                return np.atleast_1d(x)
            return x
...
def _setattr_cm(obj, **kwargs):
    ...
    for attr in kwargs:
        ...
        if attr in obj.__dict__ or orig is sentinel:
            ...
            # up after ourselves.  It is possible that this data will
...
def _premultiplied_argb32_to_unmultiplied_rgba8888(buf):
    ...
    """
    rgba = np.take(  # .take() ensures C-contiguity of the results.
        buf,
...
def _check_and_log_subprocess(command, logger, **kwargs):
    ...
    If it fails (exits with nonzero return data), raise an exception whose text
    includes the failed command and captured stdout and stderr output.

    Regardless of the return data, the command is logged at DEBUG level on
